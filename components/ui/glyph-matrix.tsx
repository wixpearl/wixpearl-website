...
export function GlyphMatrix({
  ...
  useEffect(() => {
    ...
    const [r = 107, g = 114, b = 128, a = 255] = probeCtx.getImageData(0, 0, 1, 1).data
  ...
  useEffect(() => {
    ...
    let last = 0
    let stopped = false
    const glyphPool = glyphs || DEFAULT_GLYPHS
    const randomGlyph = () => glyphPool.charAt(Math.floor(Math.random() * glyphPool.length))
    ...
    const draw = () => {
      ...
      ctx.font = `${String(cellSize - 2)}px ui-monospace, SFMono-Regular, Menlo, monospace`
